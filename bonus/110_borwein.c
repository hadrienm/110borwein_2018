...
void gauss_res(double n)
{
    ...
    double i = 1;
    for (; i < 10000.0; i++)
        resultat += fx(0.0, ((b / 2) * i + (b / 2)));
    resultat *= h * b / 2;
    printf("Gauss:\n");
    printf("I%0.0lf = %.10f\n", n, fabs(resultat - M_PI / 2) - fabs(resultat));
    printf("diff = %.10f\n\n", fabs(fabs(resultat - M_PI / 2) - fabs(resultat - M_PI / 2) - fabs(resultat)) * 0.0001);
}
